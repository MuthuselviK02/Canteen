...
try:
    ...
    if billing_response.status_code == 200:
        ...
        result = {
            "overview": {
                "total_revenue": billing_data.get("summary", {}).get("total_revenue", 0),
                "total_orders": billing_data.get("summary", {}).get("total_orders", 0),
                "avg_order_value": billing_data.get("summary", {}).get("total_orders", 0) > 0 and 
                    billing_data.get("summary", {}).get("total_revenue", 0) / billing_data.get("summary", {}).get("total_orders", 1) or 0,
                "growth_rate": billing_data.get("summary", {}).get("growth_rate", 0),
                "payment_rate": billing_data.get("summary", {}).get("paid_invoices", 0) > 0 and 
                    (billing_data.get("summary", {}).get("paid_invoices", 0) / billing_data.get("summary", {}).get("total_invoices", 1)) * 100 or 0
            }
        }
